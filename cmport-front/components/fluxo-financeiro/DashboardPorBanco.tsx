    ...
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5">
      ...
      <div className="divide-y divide-slate-100 dark:divide-slate-800">
        ...
        <div className={`flex justify-between py-1 text-sm font-black`}>
          <span className="text-slate-600 dark:text-slate-400">= Saldo final calculado (sistema)</span>
          <span className="text-slate-900 dark:text-white">
            {linha.saldo_calculado == null ? '—' : fmtValor(linha.saldo_calculado)}
          </span>
        </div>
        <div className="flex justify-between py-1 text-sm">
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h2 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-tight">Por Banco — Conciliação</h2>
          <p className="text-xs text-slate-500">Saldo inicial → entradas → saídas → saldo calculado × extrato, conta por conta</p>
        </div>
        {comImportInter && (
          <button onClick={importarInter} disabled={importando}
            className="px-3 py-2 rounded-xl text-xs font-bold bg-orange-100 dark:bg-orange-500/20 text-orange-800 dark:text-orange-300 hover:bg-orange-200 disabled:opacity-50 whitespace-nowrap">
            {importando ? 'Importando...' : 'Importar saldo Inter'}
          </button>
        )}
      </div>
      ...
      {loading ? (
        <div className="text-center py-8 text-slate-400 animate-pulse">Carregando conciliação...</div>
      ) : !dados ? (
        <div className="text-center py-8 text-slate-400">Não foi possível carregar a conciliação por banco.</div>
      ) : (
        <>
          {c && (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="bg-green-50 dark:bg-green-500/10 border border-green-200 dark:border-green-500/30 rounded-2xl p-3">
                <div className="text-[10px] font-bold text-green-700 dark:text-green-400 uppercase mb-1">Entradas (todas as contas)</div>
                <div className="text-lg font-black text-green-800 dark:text-green-300">{fmtValor(c.entradas_total)}</div>
              </div>
              <div className="bg-teal-50 dark:bg-teal-500/10 border border-teal-200 dark:border-teal-500/30 rounded-2xl p-3">
                <div className="text-[10px] font-bold text-teal-700 dark:text-teal-400 uppercase mb-1">Transferências internas</div>
                <div className="text-lg font-black text-teal-800 dark:text-teal-300">{fmtValor(c.transf_recebidas)}</div>
              </div>
              <div className="bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/30 rounded-2xl p-3">
                <div className="text-[10px] font-bold text-red-700 dark:text-red-400 uppercase mb-1">Saídas (todas as contas)</div>
                <div className="text-lg font-black text-red-800 dark:text-red-300">{fmtValor(c.saidas_total)}</div>
              </div>
              <div className={`rounded-2xl p-3 border ${c.bate ? 'bg-green-100 dark:bg-green-500/20 border-green-300' : c.diferenca == null ? 'bg-slate-900 dark:bg-slate-800 border-slate-800' : 'bg-red-100 dark:bg-red-500/20 border-red-300'}`}>
                <div className={`text-[10px] font-bold uppercase mb-1 ${c.bate ? 'text-green-700 dark:text-green-300' : c.diferenca == null ? 'text-slate-300' : 'text-red-700 dark:text-red-300'}`}>
                  {c.diferenca == null ? 'Saldo calculado' : c.bate ? 'Bate com o extrato ✓' : 'Diferença extrato'}
                </div>
                <div className={`text-lg font-black ${c.bate ? 'text-green-800 dark:text-green-200' : c.diferenca == null ? 'text-white' : 'text-red-800 dark:text-red-200'}`}>
                  {c.diferenca != null ? fmtValor(c.diferenca) : c.saldo_calculado == null ? '—' : fmtValor(c.saldo_calculado)}
                </div>
              </div>
            </div>
          )}
