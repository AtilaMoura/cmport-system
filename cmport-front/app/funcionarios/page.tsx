...
// Link pra aba "Funcionários" do Fluxo Financeiro (a folha do mês), já no mês corrente
const hojeAnoMes = () => {
  const d = new Date();
      ...
      <div className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800">
        <div className="px-4 sm:px-6 lg:px-8 py-4 lg:py-6 flex items-center justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">Cadastro, cargo, admissão e variáveis da folha</p>
          </div>
          <div className="flex items-center gap-2">
            <Link href={`/fluxo-financeiro/funcionarios?${hojeAnoMes()}`}
              className="px-4 py-2 border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-sm font-bold rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
              Ver folha do mês ↗
            </Link>
