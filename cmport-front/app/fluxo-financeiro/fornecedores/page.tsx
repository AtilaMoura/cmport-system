        ...
        {loading ? (
          ...
          <DetalheMovimentacoes movs={movs} cor="text-orange-700 dark:text-orange-400" mostrarFornecedor onAtualizado={carregar}
            breakdownExtra={cnpjsInfoFornecedor.length > 0 && (
              <div className="grid gap-3" style={{ gridTemplateColumns: `repeat(${cnpjsInfoFornecedor.length}, minmax(0, 1fr))` }}>
                {cnpjsInfoFornecedor.map(c => (
                  <div key={c.chave} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4">
                    <div className="flex items-center gap-1.5 text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">
                      {c.labelCurto}
                      <span className="px-1.5 py-0.5 rounded-full text-[10px] bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300">{c.qtd}</span>
                    </div>
                    <div className="text-2xl font-black text-orange-700 dark:text-orange-400">{fmtValor(c.total)}</div>
                  </div>
                ))}
              </div>
            )} />
