"""Extrai os blocos 'FORNECEDORES' das planilhas de fluxo financeiro
...
Mesmo padrao de extrair_despesas.py, mas pro bloco Fornecedores em vez de
Despesas Escritorio."""
import json
import openpyxl
...
def extrair(fname, cnpj_label):
    ...
    for i, row in enumerate(rows):
        ...
        if isinstance(v0, str) and v0.upper().startswith("FORNECEDORES"):
            ...
            mes_atual = v0
            continue
        # qualquer outro cabecalho de secao (MANUTEN../ASSIST../ENTRADA/DESPESAS) fecha o bloco
        if isinstance(v0, str) and any(
            v0.upper().startswith(p) for p in ("MANUTEN", "ASSIST", "ENTRADA", "DESPESAS")
        ):
            dentro_bloco = False
            continue
...
if __name__ == "__main__":
    ...
    for cnpj, fname in ARQUIVOS.items():
        ...
        print(f"{cnpj}: {len(itens)} lancamentos de fornecedores")
        todos.extend(itens)

    with open("fluxo-financeiro/fornecedores_brutos.json", "w", encoding="utf-8") as f:
        json.dump(todos, f, ensure_ascii=False, indent=2)

    print(f"TOTAL: {len(todos)} lancamentos -> fluxo-financeiro/fornecedores_brutos.json")
